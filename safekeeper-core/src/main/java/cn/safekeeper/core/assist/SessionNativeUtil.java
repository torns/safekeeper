...
public class SessionNativeUtil {
    ...
    public static boolean isExists(String sessionId) {
        return SafeKeeperManager.getTokenDao().getSession(assemblySessionKey(sessionId)) != null;
    }
    ...
    public static SafeKeeperSession getSessionById(String sessionId, boolean isCreate) {
        SafeKeeperSession safeKeeperSession = SafeKeeperManager.getTokenDao().getSession(assemblySessionKey(sessionId));
        if (safeKeeperSession == null && isCreate) {
            safeKeeperSession = SafeKeeperManager.getSaTokenAction().createSession(assemblySessionKey(sessionId));
            SafeKeeperManager.getTokenDao().setSession(safeKeeperSession, SafeKeeperManager.getConfig().getTimeout());
        }
    ...
    public static void deleteSessionById(String sessionId) {
        SafeKeeperManager.getTokenDao().deleteSession(assemblySessionKey(sessionId));
    }
}
