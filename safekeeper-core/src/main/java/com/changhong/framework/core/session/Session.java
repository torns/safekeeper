...
public class Session implements Serializable {
     ...
     */
    private final List<TokenSign> tokenList = new CopyOnWriteArrayList<>();
    ...
    public Session(String id) {
        ...
        SafeKeeperManager.getSaTokenListener().doCreateSession(id);
    }
    ...
    public TokenSign getTokenSign(String tokenValue) {
        for (TokenSign tokenSign : getTokenList()) {
    ...
    public void addTokenSign(TokenSign tokenSign) {
        // 如果已经存在于列表中，则无需再次添加
        for (TokenSign tokenSign2 : getTokenList()) {
            if (tokenSign2.getValue().equals(tokenSign.getValue())) {
                return;
            }
        }
        // 添加并更新
        getTokenList().add(tokenSign);
        update();
    }
    ...
    public void removeTokenSign(String tokenValue) {
        ...
        if (getTokenList().remove(tokenSign)) {
            update();
        }
    }
    ...
    public void update() {
        //调用底层持久化进行更新
        SafeKeeperManager.getTokenDao().updateSession(this);
    }
     ...
     */
    public void logout() {
        SafeKeeperManager.getTokenDao().deleteSession(this.id);
        SafeKeeperManager.getSaTokenListener().doLogoutSession(id);
    }
    ...
    public long getTimeout() {
        return SafeKeeperManager.getTokenDao().getSessionTimeout(this.id);
    }
    ...
    public void updateTimeout(long timeout) {
        SafeKeeperManager.getTokenDao().updateSessionTimeout(this.id, timeout);
    }
    ...
    public void updateMinTimeout(long minTimeout) {
        if(getTimeout() < minTimeout) {
            SafeKeeperManager.getTokenDao().updateSessionTimeout(this.id, minTimeout);
        }
    }
    ...
    public void updateMaxTimeout(long maxTimeout) {
        if(getTimeout() > maxTimeout) {
            SafeKeeperManager.getTokenDao().updateSessionTimeout(this.id, maxTimeout);
        }
    }
    /**
     * 取值
...
//            value = fun.run();
     ...
     * @return 对象自身
     */
    public Session setDefaultValue(String key, Object value) {
        if(has(key) == false) {
            data.put(key, value);
            update();
        }
